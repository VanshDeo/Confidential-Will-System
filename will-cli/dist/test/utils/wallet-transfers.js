...
export async function sendUnshieldedToken(wallet, address, amount) {
    const tokenTransfer = [
        {
            type: 'unshielded',
            outputs: [
    ...
    const recipe = await wallet.wallet.transferTransaction(tokenTransfer, { shieldedSecretKeys: wallet.shieldedSecretKeys, dustSecretKey: wallet.dustSecretKey }, { ttl: new Date(Date.now() + 300 * 60 * 1000) });
    const signedRecipe = await wallet.wallet.signRecipe(recipe, (payload) => wallet.unshieldedKeystore.signData(payload));
...
export async function sendArbitraryUnshieldedToken(wallet, address, amount) {
    //address Hex format
    ...
    const recipe = await wallet.wallet.balanceUnprovenTransaction(arbitraryTx, { shieldedSecretKeys: wallet.shieldedSecretKeys, dustSecretKey: wallet.dustSecretKey }, { ttl: new Date(Date.now() + 30 * 60 * 1000) });
    const signedRecipe = await wallet.wallet.signRecipe(recipe, (payload) => wallet.unshieldedKeystore.signData(payload));
    const finalizedTx = await wallet.wallet.finalizeRecipe(signedRecipe);
    const submittedTxHash = await wallet.wallet.submitTransaction(finalizedTx);
    return submittedTxHash;
}
//# sourceMappingURL=wallet-transfers.js.map