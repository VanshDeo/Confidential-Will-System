...
import { UndeployedConfig } from './config';
...
const contractMenu = (dustBalance) => `
${DIVIDER}
...
const getDustLabel = async (wallet) => {
    try {
...
const joinContract = async (providers, rli) => {
 ...
 */
const startDustMonitor = async (wallet, rli) => {
    console.log('');
 ...
 */
const deployOrJoin = async (providers, walletCtx, rli) => {
    while (true) {
        ...
        switch (choice.trim()) {
            case '1':
                try {
                    const coinPublicKey = await Rx.firstValueFrom(walletCtx.wallet.state().pipe(Rx.filter((s) => s.isSynced), Rx.map((s) => s.shielded.coinPublicKey.toHexString())));
                    const contract = await api.withStatus('Deploying will contract', () => api.deploy(providers, { beneficiaries: {}, executed: false }, coinPublicKey));
 ...
 */
const mainLoop = async (providers, walletCtx, rli) => {
        ...
        switch (choice.trim()) {
            ...
            case '3': // Claim Funds
                try {
                    ...
                    const person = await Rx.firstValueFrom(walletCtx.wallet.state().pipe(Rx.filter((s) => s.isSynced), Rx.map((s) => s.shielded.coinPublicKey.toHexString())));
