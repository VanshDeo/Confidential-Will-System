...
import { Will } from '@eddalabs/will-contract';
...
// Pre-compile the will contract with ZK circuit assets
const willCompiledContract = CompiledContract.make('will', Will.Contract).pipe(CompiledContract.withVacantWitnesses, CompiledContract.withCompiledFileAssets(contractConfig.zkConfigPath));
/**
...
export const getWillLedgerState = async (providers, contractAddress) => {
    assertIsContractAddress(contractAddress);
    logger.info('Checking contract ledger state...');
    const state = await providers.publicDataProvider
        .queryContractState(contractAddress)
        .then((contractState) => (contractState != null ? Will.ledger(contractState.data).owner : null));
    logger.info(`Ledger owner: ${state ? toHex(state) : 'null'}`);
    return state;
};
export const joinContract = async (providers, contractAddress) => {
    const willContract = await findDeployedContract(providers, {
        contractAddress,
        compiledContract: willCompiledContract,
        privateStateId: 'willPrivateState',
        initialPrivateState: { beneficiaries: {}, executed: false },
    });
    logger.info(`Joined contract at address: ${willContract.deployTxData.public.contractAddress}`);
    return willContract;
};
export const deploy = async (providers, privateState, initialOwner) => {
    logger.info('Deploying will contract...');
    const willContract = await deployContract(providers, {
        compiledContract: willCompiledContract,
        privateStateId: 'willPrivateState',
        initialPrivateState: privateState,
    });
    logger.info(`Deployed contract at address: ${willContract.deployTxData.public.contractAddress}`);
    logger.info('Initializing will contract...');
    const tx = await willContract.callTx.init(Buffer.from(initialOwner, 'hex'));
    logger.info(`Initialized contract in tx: ${tx.public.txId}`);
    return willContract;
};
export const addBeneficiary = async (willContract, person, // Coin public key hex
amount) => {
    logger.info(`Adding beneficiary ${person} with amount ${amount}...`);
    const finalizedTxData = await willContract.callTx.addBeneficiary(Buffer.from(person, 'hex'), amount);
    return finalizedTxData.public;
};
export const executeWill = async (willContract) => {
    logger.info('Executing will...');
    const finalizedTxData = await willContract.callTx.executeWill();
    return finalizedTxData.public;
};
export const claim = async (willContract, person) => {
    logger.info(`Claiming funds for ${person}...`);
    const finalizedTxData = await willContract.callTx.claim(Buffer.from(person, 'hex'));
    return finalizedTxData.public;
};
export const displayWillState = async (providers, willContract) => {
    const contractAddress = willContract.deployTxData.public.contractAddress;
    const ownerBytes = await getWillLedgerState(providers, contractAddress);
    const owner = ownerBytes ? toHex(ownerBytes) : null;
    if (owner === null) {
        logger.info(`There is no will contract deployed at ${contractAddress}.`);
    }
    else {
...
const buildShieldedConfig = ({ indexer, indexerWS, node, proofServer }) => ({
...
const buildUnshieldedConfig = ({ indexer, indexerWS }) => ({
    networkId: getNetworkId(),
    indexerClientConnection: {
        indexerHttpUrl: indexer,
        indexerWsUrl: indexerWS,
...
const buildDustConfig = ({ indexer, indexerWS, node, proofServer }) => ({
    networkId: getNetworkId(),
    costParameters: {
 ...
 */
export const withStatus = async (message, fn) => {
    const frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
    let i = 0;
...
const printWalletSummary = (seed, state, unshieldedKeystore) => {
...
`);
    ...
    return { wallet, shieldedSecretKeys, dustSecretKey, unshieldedKeystore, syncedState };
};
export const buildFreshWallet = async (config) => await buildWalletAndWaitForFunds(config, toHex(Buffer.from(generateRandomSeed())));
 ...
 */
export const monitorDustBalance = async (wallet, stopSignal) => {
    let stopped = false;
    ...
    const sub = wallet
        ...
        const availableCoins = state.dust.availableCoins.length;
        const pendingCoins = state.dust.pendingCoins.length;
        const registeredNight = state.unshielded.availableCoins.filter((coin) => coin.meta?.registeredForDustGeneration === true).length;
