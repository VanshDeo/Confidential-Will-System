...
import {
  WillCircuits,
  type WillPrivateStateId,
  type WillProviders,
  type DeployedWillContract,
} from './common-types';
...
const willCompiledContract = CompiledContract.make('will', Will.Contract).pipe(
...
export const getWillLedgerState = async (
...
export const deploy = async (
  providers: WillProviders,
  privateState: WillPrivateState,
  initialOwner: string, // Coin public key hex
): Promise<DeployedWillContract> => {
...
export const addBeneficiary = async (
  willContract: DeployedWillContract,
  person: string, // Coin public key hex
  amount: bigint,
...
export const executeWill = async (willContract: DeployedWillContract): Promise<FinalizedTxData> => {
  logger.info('Executing will...');
  const finalizedTxData = await willContract.callTx.executeWill();
  return finalizedTxData.public;
};

export const claim = async (willContract: DeployedWillContract, person: string): Promise<FinalizedTxData> => {
  logger.info(`Claiming funds for ${person}...`);
  const finalizedTxData = await willContract.callTx.claim(Buffer.from(person, 'hex'));
  return finalizedTxData.public;
};

export const displayWillState = async (
  providers: WillProviders,
  willContract: DeployedWillContract,
): Promise<{ owner: string | null; contractAddress: string }> => {
...
export const configureProviders = async (walletContext: WalletContext, config: Config) => {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<WillCircuits>(contractConfig.zkConfigPath);
  return {
    //AES-256-GCM + PBKDF2
    // WalletProvider for encryption uses Encryption Public Key (EPK)
    privateStateProvider: levelPrivateStateProvider<typeof WillPrivateStateId>({
