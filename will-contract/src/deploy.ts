/**
 * Standalone deploy script for the will contract.
 *
 * Usage:
 *   npm run build && npm run deploy
 *
 * Prompts for a hex seed (or generates a new one), builds a wallet,
 * waits for sync + funds + dust, deploys the will contract,
...
import { Contract, ledger as willLedger } from './managed/will/contract/index.js';
...
const zkConfigPath = path.resolve(currentDir, '..', 'src', 'managed', 'will');
...
// Types
// ---------------------------------------------------------------------------
type WillPrivateState = { beneficiaries: Record<string, bigint>; executed: boolean };
type WillCircuits = ImpureCircuitId<Contract<WillPrivateState>>;
const WillPrivateStateId = 'willPrivateState' as const;

// ---------------------------------------------------------------------------
// Helpers (adapted from will-cli/src/api.ts)
// ---------------------------------------------------------------------------
const frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
...
// ---------------------------------------------------------------------------
// Sign transaction intents (workaround from will-cli)
// ---------------------------------------------------------------------------
...
async function main() {
  ...
  console.log('\n╔══════════════════════════════════════════════════════════╗');
  console.log('║           Will Contract Deploy Script                    ║');
  ...
  const zkConfigProvider = new NodeZkConfigProvider<WillCircuits>(zkConfigPath);

  const providers = {
    privateStateProvider: levelPrivateStateProvider<typeof WillPrivateStateId>({
      privateStateStoreName: 'will-private-state',
  ...
  // --- Deploy ---
  const contract = await withStatus('Deploying will contract (this may take a few minutes)', async () => {
  ...
  await withStatus('Initializing will contract', async () => {
    const tx = await contract.callTx.init(Buffer.from(syncedState.shielded.coinPublicKey.toHexString(), 'hex'));
